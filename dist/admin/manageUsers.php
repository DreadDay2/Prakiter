...
<head>
    ...
    <title>PRAKTIKER - ZARZĄDZANIE UŻYTKOWNIKAMI</title>
...
<body class="bg-gray-800 justify-center items-center flex flex-col size-full overflow-x-hidden">
        ...
        if(isset($_SESSION['deleted'])){
            echo '
                <section id="announcement" class="absolute p-6 bg-gray-900 text-white rounded-xl shadow-xl flex flex-col items-center justify-center">
                    <p class="font-bold text-2xl">UŻYTKOWNIK ZOSTAŁ USUNIĘTY</p>
    ...
    <main class="w-[95vw] h-auto p-[30px] bg-gray-700 text-white flex flex-col items-center justify-center md:max-xl:flex">
        <section class="md:w-[47.5vw] w-[67.5vw] flex flex-col items-center justify-center mb-[3vh]">
            <h2 class="font-bold md:text-2xl text-[19px] md:mb-[2%] mb-[3%] text-center">Witaj w panelu zarządzania użytkownikami!</h2>
        ...
        <div class="w-full border border-gray-300 rounded-lg">
            <div class="flex flex-col md:grid grid-cols-5 bg-gray-800 p-3 text-white font-bold text-center">
                ...
                <div class="p-4">Pobieranie</div>
                <div class="p-4">Usuń</div>
                <div class="p-4">Edytuj</div>
            </div>
            <?php
                function generateTableRow($lp, $user_login, $user_permission, $user_id) {
                    return '
                        <div class="flex flex-col md:grid grid-cols-5 odd:bg-gray-600 even:bg-gray-500 text-center p-3 border-gray-400">
                            ...
                            <div class="p-2">' . htmlspecialchars($user_login) . '</div>
                            <div class="p-2">' . htmlspecialchars($user_permission) . '</div>
                            <div class="p-2">
                                <a href="deletingUser.php?id=' . htmlspecialchars($user_id) . '">
                                    <button class="outline-3 outline-solid outline-offset-2 font-bold p-4 text-emerald-400 outline-white hover:bg-gray-800 hover:outline-emerald-400 hover:text-white py-2 rounded-md w-full md:w-auto">
                                        USUŃ
                                    </button>
                                </a>
                            </div>
                            <div class="p-2">
                                <button onclick="editUser(\'' . $user_id . '\', \'' . $user_login . '\', \'' . $user_permission . '\')" 
                                    class="outline-3 outline-solid outline-offset-2 font-bold p-4 text-emerald-400 outline-white hover:bg-gray-800 hover:outline-emerald-400 hover:text-white py-2 rounded-md w-full md:w-auto">EDYTUJ DANE</button>
                            </div>
                        </div>
                    ';
                }
                ...
                while($result = mysqli_fetch_assoc($query)){
                    ...
                    $user_permission = $result['downloading'];
                    if($user_permission == "YES"){
                        $user_permission = "TAK";
                    }
                    else if($user_permission == "NO"){
                        $user_permission = "NIE";
                    }
                    echo generateTableRow($lp, $user_login, $user_permission, $result['id']);
                    $lp++;
                }
                echo "</div>";
                echo "</section>";
        ...
        <section id="edit_form" class="absolute hidden p-6 bg-gray-900 text-white rounded-xl shadow-xl">
            ...
            <form action="editingUser.php" method="post">
                ...
                <label for="new_login">
                        <span class="font-bold"><i class="fa-solid fa-user"></i>&nbsp;NOWY LOGIN:</span><br>
                        <input type="text" id="new_login" name="new_login" placeholder="Nowy login"
                ...
                <label for="new_permission">
                        <span class="font-bold"><i class="fa-solid fa-shield"></i>&nbsp;ZMIANA UPRAWNIEŃ:</span><br>
        ...
        <script>
            function editUser(id, name, permission) {
                ...
                document.getElementById('new_login').value = name;
                document.getElementById("new_permission").value = permission;
